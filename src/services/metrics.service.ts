...
export class MetricsService {
  /**
   * Whitelist of valid time periods to prevent SQL injection
   */
  private readonly VALID_PERIODS: Record<string, string> = {
    '1 hour': '1 hour',
    '24 hours': '24 hours',
    '7 days': '7 days',
    '30 days': '30 days',
  ...
  /**
   * Validate and sanitize period string
   */
  private validatePeriod(period: string): string {
    const validated = this.VALID_PERIODS[period];
    if (!validated) {
      throw new Error(`Invalid period: ${period}. Must be one of: ${Object.keys(this.VALID_PERIODS).join(', ')}`);
    }
    return validated;
  }
  ...
  async getDailyMetrics(customerId: string): Promise<DailyMetrics> {
    const period = this.validatePeriod('24 hours');
  ...
  async getWeeklyMetrics(customerId: string): Promise<DailyMetrics> {
    const period = this.validatePeriod('7 days');
  ...
  // NOTE: All methods below use validated periods from VALID_PERIODS whitelist
  // to prevent SQL injection. Period is safe to interpolate after validation.

  private async getTotalLeads(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
         AND deleted_at IS NULL`,
      [customerId]
    );
  ...
  private async getQualifiedLeads(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
  ...
  private async getRecoveredLeads(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
  ...
  private async getEstimatedRevenue(customerId: string, validatedPeriod: string): Promise<number> {
  ...
  private async getRecoveredRevenue(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ total: number }>(
      `SELECT SUM(
         COALESCE(
           CAST(SPLIT_PART(SPLIT_PART(quote->>'estimated_range', '-', 2), '$', 2) AS NUMERIC),
  ...
  private async getTopService(customerId: string, validatedPeriod: string): Promise<string | null> {
  ...
  private async getOutOfAreaCount(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
  ...
  private async getEmergencyCount(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
  ...
  private async getJunkCount(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ count: number }>(
      `SELECT COUNT(*) as count FROM leads
       WHERE customer_id = $1
         AND created_at > NOW() - INTERVAL '${validatedPeriod}'
  ...
  private async estimateAICost(customerId: string, validatedPeriod: string): Promise<number> {
    const result = await db.query<{ message_count: number; quote_count: number }>(
      `SELECT
       ...
       WHERE l.customer_id = $1
         AND l.created_at > NOW() - INTERVAL '${validatedPeriod}'
